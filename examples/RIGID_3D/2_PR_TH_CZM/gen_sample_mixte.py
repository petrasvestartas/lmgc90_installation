...
writeEvolution(f=imposeForcex, instants=numpy.linspace(0,6,60000), path='DATBOX/', name='forcex.dat')
...
def imposeForcez(t):
   ...
   if t <= t0:
     ...
     return min(2*g,g*(t-t0)/dt)

writeEvolution(f=imposeForcez, instants=numpy.linspace(0,6,60000), path='DATBOX/', name='forcez.dat')
...
body.imposeDrivenDof(description='evolution', component=1, dofty='force', evolutionFile='forcex.dat')
body.imposeDrivenDof(description='evolution', component=3, dofty='force', evolutionFile='forcez.dat')
blocks.append(body)
bodies+=body
