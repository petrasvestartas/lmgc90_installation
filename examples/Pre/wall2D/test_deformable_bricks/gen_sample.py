...
m2Dl = pre.model(name='M2D_L', physics='MECAx', element='Q4xxx', dimension=2, external_model='MatL_',
...
for j in range(0, len(mur), 1):
   ...
   for i in range(0, len(assise), 1):
      ...
      if brique.name == 'ghost': # cas de la brique fantome
         ...
         bodies += brique.deformableBrick(center=[x, y], material=steel, model=m2Dl,
                                          mesh_type='Q4', nb_elem_x=nb_elem_x, nb_elem_y=nb_elem_y)
#            nb_elem_y=nb_elem_y, apabh=[0.25, 0.75], apabv=[0.25, 0.75])
