...
# if you need to specify where is your lmgc90 package
#sys.path.insert(0,'/home/ferrante/WORK/CODES/LMGC90/lmgc90_dev/build_czm_demmefi')
...
if endo3d :
  ...
  pierre = pre.material(name='mou__',materialType='USER_MAT',density=0., file_mat='pierre_brick.mat')
  mats.addMaterial(pierre)

  pierre_m = pre.material(name='mou_m',materialType='USER_MAT',density=0., file_mat='pierre_mortar.mat')
  mats.addMaterial(pierre_m)

  m3Dl = pre.model(name='M3DNL', physics='MECAx', element='TE4xx', dimension=dim, external_model='Demfi',
  ...
  pierre = pre.material(name='Gxxxx', materialType='ELAS', elas='standard',
                     young=12.8e9, nu=0.2, anisotropy='isotropic', density=1200.)
  mats.addMaterial(pierre)

  m3Dl = pre.model(name='TET__', physics='MECAx', element='TE4xx', dimension=dim, external_model='no___',
...
mesh_wall = pre.readMesh('mesh3D.msh',dim) 
body = pre.buildMeshedAvatar(mesh=mesh_wall, model=m3Dl, material=pierre)
if body.hasGroup('Mortar'): 
   body.defineMaterial(group='Mortar',material=pierre_m)
...
for mat_file in ['pierre_brick.mat', 'pierre_mortar.mat']:
