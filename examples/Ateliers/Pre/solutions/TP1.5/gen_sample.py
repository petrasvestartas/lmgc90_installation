...
# translation de la nouvelle bille
new_ball.translate(dx=0.02)

# ajout de la nouvelle bille dans le conteneur
bodies.addAvatar(new_ball)

# boucle d'ajout des autres billes
for i in range(5):
   # creation d'une nouvelle bille par copie de la precedente
   new_ball = copy.deepcopy(new_ball)
   # rotation de la bille autour de la premiere
   new_ball.rotate(psi=math.pi/3., center=numpy.array([0.075, 0.1]))
   # ajout de la nouvelle bille au conteneur
   bodies.addAvatar(new_ball)

# on deplace le conteneur de corps pour que l'axe (0y) devienne un axe de
# symetrie
bodies.translate(dx=-0.075)
...
svdkalp = see_table(CorpsCandidat='RBDY2', candidat='DISKx',
   colorCandidat='BLEUx', behav=ldkalp, CorpsAntagoniste='MAILx', 
   antagoniste='ALpxx', colorAntagoniste='REDxx', alert=0.001)
svs.addSeeTable(svdkalp)
#   * declaration de la loi de contact entre la bille et la fondation
ldkdkx=tact_behav(name='iqsc0',law='IQS_CLB',fric=0.3)
tacts.addBehav(ldkdkx)
#   * declaration de la table de visibilite entre la billes
svdkdkx = see_table(CorpsCandidat='RBDY2', candidat='DISKx',
   colorCandidat='BLEUx', behav=ldkdkx, CorpsAntagoniste='RBDY2', 
   antagoniste='DISKx', colorAntagoniste='BLEUx', alert=0.001)
svs.addSeeTable(svdkdkx)
