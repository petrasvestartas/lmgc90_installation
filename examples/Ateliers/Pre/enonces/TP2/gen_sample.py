...
# initialisation des variables pour un depot sur un
# reseau triangulaire, avec la premiere couche de
# triangles oriente vers le bas :
nb_ele = 32   # nombre de particules sur une couche
nb_layer = 32 # nombre de couches

# calcul du nombre de particules a generer
nb_particles = 

# distribtion uiforme dans [0.01, 0.015[ 
radii=

# recuperation du plus petit et du plus grand rayon
radius_min=
radius_max=

# definition de la longueur d'un element afin que les
# particules ne se touchent pas
l = 2.5*radius_max

# calcul de la taille d'une boite englobant le reseau
[lx, ly]=

# generation la liste des coordonnees des particules
# du reseau
coor=

# boucle d'ajout des particules :
for i in range(nb_particles):
   # creation un nouveau disque rigide
   #   - rayon : rayon courant dans radii
   #   - cordonnees : coordonnees courantes dans coor
   #   - modele : mod
   #   - materiau : plex
   #   - couleur : 'BLEUx'
   body=

   # ajout du disque dans le conteneur d'avatars


# ajout du silo et de la tremie, faits de joncs :
...
# positionnement des bords de la tremie
#   * translation du centre d'inertie
#       - bord gauche : (-radius_max, -radius_max)
#       - bord droit : (lx +radius_max, -radius_max)
hopper_left
hopper_right
