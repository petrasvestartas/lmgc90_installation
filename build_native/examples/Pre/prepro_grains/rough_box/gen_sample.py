...
# center the x and y axis of the box in 0
coor[:,0] -= 0.5*lx
coor[:,1] -= 0.5*ly
...
up=pre.roughWall(center=[0.5*lx, ly + radius_max], theta=0., l=lx + 2.*radius_max, 
        r=radius_max, model=mod, material=tdur, color='WALLx')
left=pre.roughWall(center=[-radius_max, 0.5*ly], theta=-0.5*numpy.pi, l=ly + 2.*radius_max, 
        r=radius_max, model=mod, material=tdur, color='WALLx')
right=pre.roughWall(center=[lx + radius_max, 0.5*ly], theta=0.5*numpy.pi, l=ly + 2.*radius_max, 
        r=radius_max, model=mod, material=tdur, color='WALLx')
...
#       - entre particules
svdkdk = pre.see_table(CorpsCandidat='RBDY2',candidat='DISKx',
...
svdkdkb = pre.see_table(CorpsCandidat='RBDY2',candidat='DISKx',
   colorCandidat='BLEUx',behav=ldkjc, CorpsAntagoniste='RBDY2', 
   antagoniste='DISKx',colorAntagoniste='WALLx',alert=0.1*radius_min)
svs+=svdkdkb

# ecriture des fichiers
pre.writeDatbox(dim, mats, [mod], bodies, tacts, svs, datbox_path=datbox_path)
