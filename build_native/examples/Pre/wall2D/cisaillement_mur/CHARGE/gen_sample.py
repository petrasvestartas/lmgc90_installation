...
# on impose une pression sur la poutre :
# pression imposee
p = 0.3e6 # Pa
# force exercee sur la poutre
f = p*beam_length*1.

# on peut alors definir la fonction d'evolution de la force imposee sur la poutre
def imposedForce(t):
   # 0 jusqu'au temps 0
   if t <= 0.:
      return 0.
   # croissance lineaire de 0 a la force a exercer sur la poutre durant l'intervalle [0, 0.05]
   elif t > 0. and t <= 0.05:
      return -f*t/0.05
   # force maintenue a 0.05 apres
   else:
      return -f

# on peut alors ecrire le fichier d'evolution
pre.writeEvolution(f=imposedForce, instants=[0., 0.05, 10.] ,path='DATBOX/', name='fy.dat')

# on bloque la translation horizontale et la rotation de la poutre
beam.imposeDrivenDof(component=[1, 3], dofty='vlocy')
# on impose la force sur la poutre 
beam.imposeDrivenDof(description='evolution', component=2, dofty='force', evolutionFile='fy.dat')
