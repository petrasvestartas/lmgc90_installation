...
rad = 0.1
radii = rad+np.zeros([nb_particles])
...
left = pre.rigidPlan(axe1=0.5*lx, axe2=0.5*lz, axe3=rad, center=[0.,-(0.5*ly + rad), 0.5*lz],
                    model=mod, material=tdur, color='VERTx')
left.translate(dx=lx/2., dy=ly/2.)
#rotation autour de l'axe x, par rapport au centre d'inertie, avec un angle -pi/2 (parametres: angles d'Euler)
left.rotate(theta=-0.5*np.pi, center=left.nodes[1].coor)
left.imposeDrivenDof(component=[1, 2, 3, 4, 5, 6], dofty='vlocy')
bodies.addAvatar(left)

right= pre.rigidPlan(axe1=0.5*lx, axe2=0.5*lz, axe3=rad, center=[0., 0.5*ly + rad, 0.5*lz],
                    model=mod, material=tdur, color='VERTx')
right.translate(dx=lx/2., dy=ly/2.)
# rotation autour de l'axe x, par rapport au centre d'inertie, avec un angle pi/2 (parametres: angles d'Euler)
right.rotate(theta=0.5*np.pi, center=right.nodes[1].coor)
right.imposeDrivenDof(component=[1, 2, 3, 4, 5, 6], dofty='vlocy')
bodies.addAvatar(right)

front= pre.rigidPlan(axe1=0.5*lz, axe2=0.5*ly, axe3=rad, center=[0.5*lx + rad, 0., 0.5*lz],
                    model=mod, material=tdur, color='VERTx')
front.translate(dx=lx/2., dy=ly/2.)
# rotation autour de l'axe y, par rapport au centre d'inertie, avec un angle -pi/2 (parametres: axe + angle)  
front.rotate(description='axis', alpha=-0.5*np.pi, axis=[0., 1., 0.], center=front.nodes[1].coor)
front.imposeDrivenDof(component=[1, 2, 3, 4, 5, 6], dofty='vlocy')
bodies.addAvatar(front)

rear = pre.rigidPlan(axe1=0.5*lz, axe2=0.5*ly, axe3=rad, center=[-(0.5*lx + rad), 0., 0.5*lz],
                    model=mod, material=tdur, color='VERTx')
rear.translate(dx=lx/2., dy=ly/2.)
# rotation autour de l'axe y, par rapport au centre d'inertie, avec un angle pi/2 (parametres: axe + angle)
rear.rotate(description='axis', alpha=0.5*np.pi, axis=[0., 1., 0.], center=rear.nodes[1].coor)
