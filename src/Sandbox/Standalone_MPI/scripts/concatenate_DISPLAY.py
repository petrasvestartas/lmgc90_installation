...
for i in range(nb_display):
    ...
    for j in range(nb_SDM):
        my_inputs.append("DDM_WD_"+"%(#)05d" % {"#": j+1}+"/DISPLAY/a_ibdy2_"+"%(#)07d" % {"#": i+1}+".vtu")

    my_output=open("DISPLAY/a_ibdy2_"+"%(#)07d" % {"#": i+1}+".vtu","w")
...
for i in range(nb_display):
    my_inputs=[]
    for j in range(nb_SDM):
        my_inputs.append("DDM_WD_"+"%(#)05d" % {"#": j+1}+"/DISPLAY/a_ibdy3_"+"%(#)07d" % {"#": i+1}+".vtu")

    my_output=open("DISPLAY/a_ibdy3_"+"%(#)07d" % {"#": i+1}+".vtu","w")
    for my_file in my_inputs:
        my_input=open(my_file, "r")
