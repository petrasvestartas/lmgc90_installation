program PRPR_standalone_DDM_MPI_DECENT
...
integer :: id_read,id_free,id_nlgs_prep,id_nlgs_iter,id_crea_dom,id_detect_fine, &
           ...
           id_nlgs_vfree, id_ddm_recoll, id_Vfree_list
  ...
  real(kind=8)     :: Vref             ! Vitesse de reference du probleme considere
 
  real(kind=8)     :: egluing          ! Erreur de recollement des grains d'interface normalisee 
  ...
  integer          :: iddm         ! indice d'iteration ddm

  !real(kind=8)     :: Xleft_bound, Xright_bound ! Frontieres gauche, droite de la boite anglobante 
  !                                              ! On contraint donc le decoupage sous-jacent suivant x.
...
! </read
       ...
       ! Pour l'ecriture des DOF, active la prise en compte de
       ! la visibilite des corps dans la fonction write_out_dof.
...
! read/>
       ...
       !   * allocation des tableaux du processus hote.
       !---> Fonction rang0 <---
       call new_dd_host 

       !---> Fonction rang0 <---
       ! triax_compac doit TOUJOURS etre a .false. en PRPR !!
...
! </free
          ...
          !     on ne fait cela ici que pour le move_polyr...
...
! free/>
          ...
          if ( RUN_PRPRx() ) then
             ! On ne peut merger les donnees que si des calculs
             ! ont deja ete faits et on ne veut merger les donnees
             ! que lorsqu'une nouvelle repartition en sous-domaines 
             ! vas avoir lieu.
             ...
             ! On rend visible la premiere tranche en vue de la nouvelle repartition en sous-domaines
             ! => recuperation des bonnes coordonnees des centre d'intertie des corps
             ! processus hote (-->0)
             ...
             call scatter_creation_domaines ! Scatters et preparations de la BDD
             ...
             ! Calcul et set des masses modifiees par les differents processus
          ...
          ! Effectue compute_configurationTT_RBDY3, c'est bizarre...
...
! </detect
          ...
          if ( check_PRPRx() ) then
             ! cf commentaire sur comp_free_vlocy_RBDY3
             ! call compute_configurationTT_RBDY3
             CALL coor_prediction_PRPRx

             ! Mise en donnee dans mod_PRPRx du graphe de contact grossier
          ...
          ! recuperation dans le module DDM de la liste des contacts tagges 'INTRF'.
     ...
     ddm2:do iddm=1,itloop2
        ...
        ddm1:do ik=1,itloop1
                ...
                if (NSCDD) then 
                   ...
                   ! Échanges decentralises des torseurs
...
end program PRPR_standalone_DDM_MPI_DECENT
