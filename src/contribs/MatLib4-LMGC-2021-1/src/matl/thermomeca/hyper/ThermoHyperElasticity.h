/*
 *  $Id: ThermoHyperElasticity.h 266 2020-03-31 14:42:08Z lstainier $
 *
...
/**
 * Base class for (isotropic) thermo-hyperelastic potentials,
 * which are function of principal stretches.
 */
template <class ALG>
class SpectralThermoHEPotential : virtual public HyperElasticity<ALG>::Potential {
 ...
 protected:
  ...
  // constructor
  SpectralThermoHEPotential() {}

 public:

  // destructor
  virtual ~SpectralThermoHEPotential() {}
  ...
  double storedThMEnergy(const MaterialProperties& material,const ParameterSet& extPar,
    ...
    double lambda[3],t[3],h[3][3],dt[3];
    SYM_TENSOR NN[3];
    C.eigenSplit(lambda,NN);

    // compute stored energy
    double W = storedThMEnergy(material,extPar,lambda,T,t,N,h,dt,Cm,
                               first || second,second);

    // stresses
    if (first) {
      S = 0.0e0;
      for (unsigned int k=0; k < 3; k++) S += (2*t[k])*NN[k];
    }
      ...
      for (unsigned int k=0; k < 3; k++)
        for (unsigned int l=0; l < 3; l++) {
          ...
          if (std::fabs(dl) > 1.e-16)
            ...
            coef = 2*(h[l][l]-h[k][l]);
          M.addIJKL(coef,NN[k],NN[l]);
        }
      dS = 0.0e0;
      for (unsigned int k=0; k < 3; k++) dS += (2*dt[k])*NN[k];
    }
  ...
  virtual double storedThMEnergy(const MaterialProperties&,const ParameterSet&,
                                 ...
                                 double[][3],double[],double&,bool,bool) = 0;
};
