/*
 *  $Id: CompositeHyperElasticity.h 140 2013-08-30 15:38:09Z lstainier $
 *
 ...
 */
#ifndef ZORGLIB_MATL_MECA_HYPER_COMPOSITE_ELASTICITY_H
#define ZORGLIB_MATL_MECA_HYPER_COMPOSITE_ELASTICITY_H
...
/**
 * Base class for composite (multi-material) hyper-elasticity models.
 */
template <class ALG>
class CompositeHyperElasticity : virtual public StandardMaterial {
 ...
 protected:
  ...
  std::vector<HyperElasticity<ALG>*> phases;
  
  // instance counter
  unsigned int *count;
  
  // empty constructor
  CompositeHyperElasticity() {
 ...
 public:
  ...
  // constructor
  CompositeHyperElasticity(std::vector<HyperElasticity<ALG>*>& p) {
    count = new unsigned int(1);
    phases = p;
  }

  // copy constructor
  CompositeHyperElasticity(const CompositeHyperElasticity& src) {
    count = src.count;
    (*count)++;
    phases = src.phases;
  }
  ...
  // destructor
  virtual ~CompositeHyperElasticity() {
    if (--(*count) > 0) return;
    delete count;
  ...
  void checkProperties(MaterialProperties& material,std::ostream* os = 0) 
   throw (InvalidPropertyException, NoSuchPropertyException) {
    if (os) (*os) << "\nFinite-strains composite material:" << std::endl;
  ...
  void checkPhases(MaterialProperties& material,std::ostream* os = 0)
   throw (InvalidPropertyException, NoSuchPropertyException) {
    ...
    for (unsigned int i=0; i < phases.size(); i++) {
      if (os) (*os) << "\nHyper-elastic variant #" << i+1 << ":" << std::endl;
      if (os) (*os) << "---------------------------------" << std::endl;
