/*
 *  $Id: logsym1.cpp 124 2013-01-11 16:41:33Z lstainier $
 *
 ...
 */
int logsym2_series(const double A[],double logA[],double dLogA[][3],double d2LogA[][3][3],
  ...
  double A1[3],X[3],dX[3][3],d2X[3][3][3],Y[3],dY[3][3],d2Y[3][3][3];

  /*
   * Initialization
   */
  std::memcpy(A1,A,3*sizeof(double));
  A1[0] -= 1.e0; A1[2] -= 1.e0;
  std::memcpy(X,A1,3*sizeof(double));
  std::memcpy(logA,X,3*sizeof(double));

  if (computeFirst || computeSecond) {
    std::memset(dX,0,9*sizeof(double));
    dX[0][0] = dX[2][2] = 1.0e0;
    dX[1][1] = 0.5e0;
    std::memcpy(dLogA,dX,9*sizeof(double));
  }
  ...
  if (computeSecond) {
    ...
    std::memcpy(d2LogA,d2X,27*sizeof(double));
  }
  ...
  for (int iter=1; iter < ITMAX; iter++) {
    ...
    if (computeSecond && iter >= 2) {
        ...
        for (l=0; l <= k; l++, kl++) 
          for (m=0, mn=0; m < 2; m++)
            for (n=0; n <= m; n++, mn++) {
              ...
              symsym2(d2Y[kl][mn],A1,d2X[kl][mn]);
      ...
      if (computeSecond) addvec(d2LogA[0][0],d2X[0][0],d2LogA[0][0],27);
    ...
    if ((computeFirst || computeSecond) && iter >= 1) {
      ...
      if (computeFirst) addvec(dLogA[0],dX[0],dLogA[0],9);
    ...
    symsym2(Y,A1,X); // X = Y*A (X still symmetric in this case)
    mulvec(coef,X,X,3);
    addvec(logA,X,logA,3);
 ...
 * using truncated series
 */
int logsym3_series(const double A[],double logA[],double dLogA[][6],double d2LogA[][6][6],
                   bool computeFirst,bool computeSecond) {
  ...
  int i,j,k,l,m,n,ij,kl,mn;
  double A1[6],X[6],dX[6][6],d2X[6][6][6],Y[6],dY[6][6],d2Y[6][6][6];

  /*
   * Initialization
   */
  std::memcpy(A1,A,6*sizeof(double));
  A1[0] -= 1.e0; A1[2] -= 1.e0; A1[5] -= 1.e0;
  std::memcpy(X,A1,6*sizeof(double));
  std::memcpy(logA,X,6*sizeof(double));

  if (computeFirst || computeSecond) {
    std::memset(dX,0,36*sizeof(double));
    dX[0][0] = dX[2][2] = dX[5][5] = 1.0e0;
    dX[1][1] = dX[3][3] = dX[4][4] = 0.5e0;
    std::memcpy(dLogA,dX,36*sizeof(double));
  }
  ...
  if (computeSecond) {
    ...
    std::memcpy(d2LogA,d2X,216*sizeof(double));
  }
  ...
  for (int iter=1; iter < ITMAX; iter++) {
    ...
    if (computeSecond && iter >= 2) {
        ...
        for (l=0; l <= k; l++, kl++) 
          for (m=0, mn=0; m < 3; m++)
            for (n=0; n <= m; n++, mn++) {
              ...
              // the sum of the three contributions is symmetric,
              // but not the contribution themselves !
              symsym3(d2Y[kl][mn],A1,d2X[kl][mn]);
      ...
      if (computeSecond) addvec(d2LogA[0][0],d2X[0][0],d2LogA[0][0],216);
    ...
    if ((computeFirst || computeSecond) && iter >= 1) {
      ...
      if (computeFirst) addvec(dLogA[0],dX[0],dLogA[0],36);
    ...
    symsym3(Y,A1,X); // X = Y*A (X still symmetric in this case)
    mulvec(coef,X,X,6);
    addvec(logA,X,logA,6);
