/*
 *  $Id: ConvexHyperElasticity.h 140 2013-08-30 15:38:09Z lstainier $
 *
 ...
 */
#ifndef ZORGLIB_MATL_MECA_HYPER_CONVEX_ELASTICITY_H
#define ZORGLIB_MATL_MECA_HYPER_CONVEX_ELASTICITY_H
...
#include <vector>
// local
#include <matl/meca/hyper/CompositeHyperElasticity.h>
...
/**
 * Base class for (poly)convexified multi-well hyper-elasticity models.
 */
template <class ALG>
class ConvexHyperElasticity : virtual public CompositeHyperElasticity<ALG> {
 ...
 protected:
  ...
  // empty constructor
  ConvexHyperElasticity() {}

 public:

  // constructor
  ConvexHyperElasticity(std::vector<HyperElasticity<ALG>*>& w)
  : CompositeHyperElasticity<ALG>(w) {}

  // copy constructor
  ConvexHyperElasticity(const ConvexHyperElasticity& src) 
  : CompositeHyperElasticity<ALG>(src) {}
  
  // destructor
  virtual ~ConvexHyperElasticity() {}
  
  // check consistency of material properties
  void checkProperties(MaterialProperties& material,std::ostream* os = 0) 
   throw (InvalidPropertyException, NoSuchPropertyException) {
    if (os) (*os) << "\nPolyconvexified multi-well hyper-elastic material:" << std::endl;
  ...
  double incrementalPotential(const MaterialProperties& material,
   ...
   throw (UpdateFailedException) {
    ...
    // polar decomposition
    unsigned int T_dim = 2*SYM_TENSOR::MEMSIZE+1;
    MatLibArray TU(T_dim);
    SYM_TENSOR U(TU);
    TENSOR R;
    ALG::RUDecomposition(F,R,U);
    std::cout << "U=" << U << std::endl;
     
    // compute cofactor and determinant
    SYM_TENSOR coU(TU,SYM_TENSOR::MEMSIZE);
    ALG::SymTensor::cofactor(U,coU);
    U = covariant(U);
    coU = covariant(coU);
    TU[2*SYM_TENSOR::MEMSIZE] = determinant(U);
    ...
    MatLibArray TUe[sz],TS(T_dim);
    MatLibMatrix TTe[sz];
    MaterialProperties m[sz];
     
    // initialization
    TS = 0.0e0; /* IMPROVE LATER */
     //TS[0] = TS[1] = TS[2] = TS[3] = 1.0e0;
     //TS[4] = TS[5] = TS[6] = TS[7] = 1.0e0;
     //TS[8] = 1.0e0;
    unsigned int i;
    for (i=0; i < sz; i++) {
      m[i].clear();
      ...
      TUe[i] = TU;
      TTe[i].resize(T_dim);
      w[i] = this->phases[i]->dualEnergy(m[i],extPar,TS,TUe[i],TTe[i],update,update || tangent);
      /*{
        MatLibArray TUp(T_dim),TUm(T_dim),TUnum(T_dim);
        MatLibMatrix TTnum(T_dim);
        for (unsigned int k=0; k < T_dim; k++) {
          TS[k] += 1.e-6;
          TUp = TU;
          double wp = this->phases[i]->dualEnergy(m[i],extPar,TS,TUp,TTnum,true,false);
          TS[k] -= 2.e-6;
          TUm = TU;
          double wm = this->phases[i]->dualEnergy(m[i],extPar,TS,TUm,TTnum,true,false);
          TS[k] += 1.e-6;
          TUnum[k] = (wp-wm)*0.5e6;
          for (unsigned l=0; l < T_dim; l++)
            TTnum[l][k] = (TUp[l]-TUm[l])*0.5e6;
        }
        std::cout << "ana.=" << TUe[i] << std::endl;
        std::cout << "num.=" << TUnum << std::endl;
        std::cout << "ana.=" << TTe[i] << std::endl;
        std::cout << "num.=" << TTnum << std::endl;
      }*/
      if (i == 0)
        wMax = w[0];
      else
    ...
    for (i=0; i < sz; i++) {
      l[i] *= Zinv;
      std::cout << "\ti=" << i+1 << ",W*=" << w[i] << ",T(F)=" << TUe[i] << ",l=" << l[i] << std::endl;
    }
    ...
    if (update) {
      ...
      while (iter < ITMAX) {
        ...
        // evaluate residual
        MatLibArray res = TU;
        for (i=0; i < sz; i++) res -= l[i]*TUe[i];
        double norm1 = normL2(res);
        std::cout << "iter=" << iter << "-R=" << res << "-norm=" << norm1 << "(norm0=" << norm0 << ")" << std::endl;
        ...
        // solve
        MatLibMatrix TT(T_dim);
        for (i=0,TT=0.0e0; i < sz; i++)
          TT += l[i]*(TTe[i]+beta*outerProd(TUe[i],TUe[i]+res-TU));
        std::cout << TT << std::endl;
        MatLibArray dTS(T_dim);
        TT.solve(dTS,res,true);
        ...
        // update values
        for (i=0; i < sz; i++) {
          w[i] = this->phases[i]->dualEnergy(m[i],extPar,TS,TUe[i],TTe[i],true,true);
          /*{
           MatLibArray TUp(T_dim),TUm(T_dim),TUnum(T_dim);
           MatLibMatrix TTnum(T_dim);
           for (unsigned int k=0; k < T_dim; k++) {
      ...
      if (iter == ITMAX) throw UpdateFailedException("polyconvexification");
    }
     
    // compute energy envelope
    double W = innerProd(TU,TS)-std::log(Z)/beta-wMax;
